...
export default function SchoolAlertDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchAlertDetails = async () => {
      try {
        // Fetch alert details
        const alertResponse = await fetch(`/api/alerts/${params.id}`);
        if (!alertResponse.ok) {
        ...
        // Fetch child details
        if (alertData.childId) {
          const childResponse = await fetch(`/api/children/${alertData.childId}`);
          if (childResponse.ok) {
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            View alert information for student
          </p>
        </div>
        <Link
          href="/dashboard/school/alerts"
